...
namespace QL_DiemTHPT
{
    public partial class FrmNHAPDIEMMON : Form
    {
        ...
        private void HienThiBang()
        {
            ...
            string sql = string.Format(
                @"SELECT d.MADIEM, hs.MAHS AS [Mã HS], hs.HOTEN AS [Họ tên],
        ...
        private void dgvDiem_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtDIEMTX.Text = row.Cells["ĐTX"].Value?.ToString();
            txtDIEMGK.Text = row.Cells["ĐGK"].Value?.ToString();
            txtDIEMCK.Text = row.Cells["ĐCK"].Value?.ToString();
        ...
        private void dgvDiem_CurrentCellDirtyStateChanged(object sender, EventArgs e)
        {
            if (dgvDiem.IsCurrentCellDirty)
                dgvDiem.CommitEdit(DataGridViewDataErrorContexts.Commit);
        }
